=begin
Alphabet war - airstrike - letters massacre by Vladislav Trotsenko.

There is a war and nobody knows - the alphabet war!
There are two groups of hostile letters. The tension between left side letters
and right side letters was too high and the war began.

Write a function that accepts fight string consists of only small letters and
return who wins the fight. When the left side wins return Left side wins!,
when the right side wins return Right side wins!, in other case return Let's
fight again!.

The left side letters and their power:
w - 4
p - 3 
b - 2
s - 1

The right side letters and their power:
m - 4
q - 3 
d - 2
z - 1

The other letters don't have power and are only victims.
The * bombs kills the adjacent letters ( i.e. aa*aa => a___a, **aa** => ______ );

Examples:
AlphabetWar("s*zz");           //=> Right side wins!
AlphabetWar("*zd*qm*wp*bs*"); //=> Let's fight again!
AlphabetWar("zzzz*s*");       //=> Right side wins!
AlphabetWar("www*www****z");  //=> Left side wins!
=end

def alphabet_war(fight)
  total_left = total_right = 0
  left, right, bomb, without_bombs = %w(s b p w), %w(z d q m), '*', ''
    unless fight.include?(bomb)
      without_bombs = fight
    else
      fight.each_char.with_index do |char, index|
        pred_char, next_char = fight[index-1], fight[index+1]
          next if char == bomb || (index != fight.size-1 && next_char == bomb) || (!index.zero? && pred_char == bomb)
        without_bombs << char
      end
    end
    [left, right].each_with_index do |arr, arr_index|
      dict = arr.map.with_index { |item, index| [item, index+1] }.to_h
        without_bombs.each_char do |char|
          arr_index.zero? ? total_left+=dict[char] : total_right+=dict[char] if dict.has_key?(char)
        end
    end
    winner = case
        when total_left > total_right then 'Left side wins!'
        when total_left < total_right then 'Right side wins!'
        else "Let's fight again!"
      end
    winner
end

alphabet_war('mb**qwwp**dm')