=begin
Validate Credit Card Number by Vladislav Trotsenko.
...
Finally, take that sum and divide it by 10. If the remainder equals zero, the original
...
def validate(n)
  n = n.to_s.chars.map(&:to_i)
    n.map.with_index { |num, index| sum = num*2
      n.size.even? && (index+1).odd? || n.size.odd? && index.odd? ? (sum > 9 ? sum-9 : sum) : num
    }.inject(:+)%10 == 0 ? true : false
end

validate(5167460010723058)