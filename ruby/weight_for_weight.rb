=begin
Weight for weight by Vladislav Trotsenko.
...
"weight" (1) is less than the one of 180 (9) and 180 is before
...
def order_weight(string)
  string.split.sort_by { |item| [item.chars.map(&:to_i).inject(:+), item] }.join(' ')
end
